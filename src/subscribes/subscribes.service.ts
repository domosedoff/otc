...
import { Injectable, NotFoundException } from '@nestjs/common'; // <-- Убран BadRequestException
...
export class SubscribesService {
  private readonly COST_PER_DAY = 1.0; // $1.00 за день размещения (константа для MVP)
  ...
  async activateSubscription(
    ...
    let endDate = addDays(today, duration_days); // <-- ИСПРАВЛЕНИЕ: 'let' вместо 'const'
